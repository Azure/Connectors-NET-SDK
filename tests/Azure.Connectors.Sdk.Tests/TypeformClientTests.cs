...
{
    [TestClass]
    public class TypeformClientTests
    {
        ...
        {
            using var client = new TypeformClient("https://test.azure.com/connection");
        ...
        {
            Assert.ThrowsExactly<ArgumentNullException>(() => new TypeformClient((string)null!));
        }

        [TestMethod]
        public void Dispose_ShouldNotThrow()
        {
            var client = new TypeformClient("https://test.azure.com/connection");
            client.Dispose();
        }
        ...
        [TestMethod]
        public async Task ListFormsAsync_WithMockedResponse_ReturnsExpected()
        {
        ...
        [TestMethod]
        public async Task GetSchemaAsync_WithErrorResponse_ThrowsConnectorException()
        {
            using var responseMessage = new HttpResponseMessage
            {
                StatusCode = HttpStatusCode.BadRequest,
            ...
            await Assert.ThrowsExactlyAsync<ConnectorException>(() =>
                client.GetSchemaAsync(form: "form1",
                    cancellationToken: CancellationToken.None))
                .ConfigureAwait(continueOnCapturedContext: false);
        }
