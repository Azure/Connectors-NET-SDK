...
namespace Azure.Connectors.Sdk.Tests
{
    ...
    public class SlackClientTests
    {
        ...
        private static SlackClient CreateMockedClient(HttpResponseMessage response)
        {
        ...
        {
            using var client = new SlackClient("https://test.azure.com/connection");
        ...
        {
            Assert.ThrowsExactly<ArgumentNullException>(() => new SlackClient((string)null!));
        }

        [TestMethod]
        public void Dispose_ShouldNotThrow()
        {
            var client = new SlackClient("https://test.azure.com/connection");
            client.Dispose();
        }
        ...
        [TestMethod]
        public async Task SetDNDAsync_WithMockedResponse_ReturnsExpected()
        {
            ...
            var result = await client
                .SetDNDAsync(cancellationToken: CancellationToken.None)
        ...
        [TestMethod]
        public async Task SetDNDAsync_WithErrorResponse_ThrowsConnectorException()
        {
            using var responseMessage = new HttpResponseMessage
            {
            ...
            await Assert.ThrowsExactlyAsync<ConnectorException>(() =>
                client.SetDNDAsync(cancellationToken: CancellationToken.None))
                .ConfigureAwait(continueOnCapturedContext: false);
        }
