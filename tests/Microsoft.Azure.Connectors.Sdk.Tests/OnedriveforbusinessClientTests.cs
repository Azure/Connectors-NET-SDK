...
namespace Microsoft.Azure.Connectors.Sdk.Tests
{
    ...
    public class OnedriveforbusinessClientTests
    {
        ...
        [TestMethod]
        public async Task ListFolderAsync_SinglePage_ReturnsAllItems()
        {
            // Arrange
            var page = new BlobMetadataPage
            {
                ...
                {
                    new BlobMetadata { Id = "file-1", Name = "doc1.pdf", IsFolder = false },
                    new BlobMetadata { Id = "file-2", Name = "doc2.pdf", IsFolder = false }
                },
                NextLink = null
            };
            ...
            // Act
            var items = new List<BlobMetadata>();
            await foreach (var item in client.ListFolderAsync("root"))
            {
                items.Add(item);
            }
            ...
            Assert.AreEqual("file-1", items[0].Id);
            Assert.AreEqual("file-2", items[1].Id);
        }

        [TestMethod]
        public async Task ListFolderAsync_MultiplePages_FollowsNextLink()
        {
            // Arrange
            var page1 = new BlobMetadataPage
            {
                Value = new List<BlobMetadata>
                {
                    new BlobMetadata { Id = "file-1", Name = "doc1.pdf", IsFolder = false }
                },
                NextLink = "https://test.azure.com/connection/datasets/default/foldersV2/root?$skiptoken=page2"
            };

            var page2 = new BlobMetadataPage
            {
                Value = new List<BlobMetadata>
                {
                    new BlobMetadata { Id = "file-2", Name = "doc2.pdf", IsFolder = false }
            ...
            mockHandler.Protected()
                ...
                .ReturnsAsync(() =>
                {
                    ...
                    var responseBody = callCount == 1
                        ? JsonSerializer.Serialize(page1)
                        : JsonSerializer.Serialize(page2);
        ...
        [TestMethod]
        public async Task ListFolderAsync_AsPages_YieldsPageObjects()
        {
            // Arrange
            var page1 = new BlobMetadataPage
            {
                ...
                NextLink = "https://test.azure.com/connection/next?skip=1"
            };
            ...
            // Act
            var pages = new List<BlobMetadataPage>();
            ...
            Assert.AreEqual("https://test.azure.com/connection/next?skip=1", pages[0].NextLink);
            Assert.IsNull(pages[1].NextLink);
        }

        [TestMethod]
        public async Task ListFolderAsync_NextLink_UsesAbsoluteUrl()
        {
            // Arrange — Verify the second request uses the absolute NextLink URL, not a relative path
            var page1 = new BlobMetadataPage
            {
                Value = new List<BlobMetadata>
                {
                ...
                NextLink = "https://different-host.azure.com/next?skipToken=abc"
            };
            ...
            // Assert — second request should use the absolute NextLink, not prepend connectionRuntimeUrl
            Assert.AreEqual(2, requestUrls.Count);
            Assert.IsTrue(requestUrls[0].StartsWith("https://test.azure.com/connection/", StringComparison.Ordinal));
            Assert.AreEqual("https://different-host.azure.com/next?skipToken=abc", requestUrls[1]);
        }
    }
}
