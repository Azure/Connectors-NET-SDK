...
- **Plumsail Timezone enum: all signed Etc/GMT members now have semantic Plus/Minus names** — the fix renames all 27 signed Etc/GMT Timezone members to use readable `Plus`/`Minus` sign words rather than numeric discriminator suffixes. Callers of the old numeric names (`EtcGMT1`, `EtcGMT4`, `EtcGMT12`, `EtcGMT42`, `EtcGMT15`, etc.) must rename to the corresponding semantic name: `EtcGMT+1` → `EtcGMTPlus1`, `EtcGMT-1` → `EtcGMTMinus1`, `EtcGMT+4` → `EtcGMTPlus4`, `EtcGMT-4` → `EtcGMTMinus4`, and so on. Three-way group `+0`/`-0`/`0` maps to `EtcGMTPlus0`, `EtcGMTMinus0`, `EtcGMT0`. Singleton entries without a signed counterpart (`Etc/GMT-13` → `EtcGMT13`, `Etc/GMT-14` → `EtcGMT14`) are unchanged. The underlying wire strings are preserved in all cases. ([#181](https://github.com/Azure/Connectors-NET-SDK/issues/181))
