...
- **Array item types are documented from their own definition instead of the collection envelope that wraps them** — a type reached through a bare envelope property such as `value` was summarized as `Item in value`, naming the wrapper rather than the element. Zendesk `Table` and `Item` now carry their own names, and seven SQL Server item types gain the descriptions their Swagger definitions already declared, for example `Item in List of datasets` becomes `Represents a database.` Documentation only; no operation, model, or wire change. (AzureUX-BPM PR 16600581)
