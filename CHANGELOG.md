...
- **SigningHub upload methods now require file bytes** — `AttachmentUploadAttachmentAsync` and `DocumentsUploadStreamAsync` now require a `byte[] input` argument and send it as the request body, matching their required `application/octet-stream` Swagger body parameters. Existing callers must provide the attachment or document bytes. (#210)
