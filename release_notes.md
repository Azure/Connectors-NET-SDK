...
- **Plumsail Timezone enum: all signed Etc/GMT members now have semantic Plus/Minus names** — the fix renames all 27 signed Etc/GMT Timezone members to use readable `Plus`/`Minus` sign words rather than numeric discriminator suffixes. Callers must rename old numeric discriminator identifiers to the corresponding semantic name. For example, the old identifier `EtcGMT4` (for `Etc/GMT+4`) becomes `EtcGmtPlus4`, and the old identifier `EtcGMT42` (for `Etc/GMT-4`) becomes `EtcGmtMinus4`; similarly `EtcGMT1` → `EtcGmtPlus1`, `EtcGMT12` → `EtcGmtMinus1`, and so on for each signed pair. Three-way group (`EtcGMT0`, `EtcGMT02`, `EtcGMT03`) maps to `EtcGmtPlus0`, `EtcGmtMinus0`, and `EtcGmt0`. Singleton entries without a signed counterpart (`Etc/GMT-13` → `EtcGMT13`, `Etc/GMT-14` → `EtcGMT14`) are unchanged. The underlying wire strings are preserved in all cases. ([#181](https://github.com/Azure/Connectors-NET-SDK/issues/181))
