...
- **Extensible enum member normalization collision now handled correctly** — the generator had two related defects. *(1) Normalization collision*: when two distinct swagger enum wire values normalize to the same C# identifier (for example, `Etc/GMT+4` and `Etc/GMT-4` both → `EtcGMT4` after sign characters are treated as delimiters), the second claimant was silently dropped. The hardened two-phase assignment (AzureUX-BPM PR 16971205, commits d5cb672..87a77e5) pre-scans natural names in phase 1, then in phase 2 tries semantic disambiguation before falling back to numeric. When colliding values differ by a sign character immediately followed by a digit, the sign is expanded to a readable word (`+` → `Plus`, `-` → `Minus`), giving the whole collision group distinct semantic names (`Etc/GMT+4` → `EtcGMTPlus4`, `Etc/GMT-4` → `EtcGMTMinus4`). Semantic override is blocked if any resulting name would conflict with another value's natural identifier, in which case numeric discriminators are used instead. Ordinary delimiter hyphens (`my-value` → `MyValue`) are unaffected. Three-way groups (`+N`/`-N`/unsigned `N`) work: the unsigned member keeps its natural name (`Etc/GMT0`), the signed members receive `Plus`/`Minus` names (`EtcGMTPlus0`, `EtcGMTMinus0`). *(2) Reserved name collision*: when a swagger value normalized to a mandatory struct member name (`Equals`, `GetHashCode`, `ToString`), the generator shadowed the required override method and produced CS0102. Reserved names are instead suffixed with `Value`. An empirical scan of all 100 checked-in connector clients against the frozen 2026-08-28 ARM swagger snapshot (cache manifest SHA256 `ED9C3FB8911D0F28C8D5038CE33ABEE38F7265D186249C8C66CC5C4B7B7CF2C8`) found one connector with active normalization collisions in the current swagger: Plumsail (Timezone enum). Plumsail is regenerated; the remaining 99 clients are unchanged. ([#181](https://github.com/Azure/Connectors-NET-SDK/issues/181))

  The reserved-name defect is addressed as a preventive guard. The eleven connectors originally cited in the issue (Blackbaudaltruconsti, Blackbaudcrmconstitu, Dataflows, Documentscorepackapi, Dynamicssmbsaas, Etsy, Iaconnectmsoffice, Meisterplan, Powerassist, Tabscannerreceiptocr, Workpoint365) are no longer present in the current ARM connector catalog; Etsy is present but its current swagger carries no reserved-name collision.

#### Breaking Changes

- **Plumsail Timezone enum: all signed Etc/GMT members now have semantic Plus/Minus names** — the fix renames all 27 signed Etc/GMT Timezone members to use readable `Plus`/`Minus` sign words rather than numeric discriminator suffixes. Callers of the old numeric names (`EtcGMT1`, `EtcGMT4`, `EtcGMT12`, `EtcGMT42`, `EtcGMT15`, etc.) must rename to the corresponding semantic name: `Etc/GMT+1` → `EtcGMTPlus1`, `Etc/GMT-1` → `EtcGMTMinus1`, `Etc/GMT+4` → `EtcGMTPlus4`, `Etc/GMT-4` → `EtcGMTMinus4`, and so on. Three-way group `+0`/`-0`/`0` maps to `EtcGMTPlus0`, `EtcGMTMinus0`, `EtcGMT0`. Singleton entries without a signed counterpart (`Etc/GMT-13` → `EtcGMT13`, `Etc/GMT-14` → `EtcGMT14`) are unchanged. The underlying wire strings are preserved in all cases. ([#181](https://github.com/Azure/Connectors-NET-SDK/issues/181))
